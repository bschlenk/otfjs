...
const [
  ,
  ,
  fontPath = 'fonts/google/Roboto.ttf',
  glyphIndexStr = '39',
  fontSizeStr = '16',
] = process.argv
...
try {
  ...
  const record =
    hmtx.longHorMetrics[glyphIndex] ??
...
console.log(
  `[hint] font=${fontPath} glyph=${glyphIndex} fontSize=${fontSize} upem=${upem}`,
)

try {
  vm.runFpgm()
  console.log(
    '[hint] cvt[0..19] after fpgm:',
    vm.cvt
      .slice(0, 20)
      .map((v, i) => `${i}:${(v ?? 0).toFixed(0)}`)
      .join(' '),
  )

  vm.runPrep()
  console.log(
    '[hint] cvt[0..19] after prep:',
    vm.cvt
      .slice(0, 20)
      .map((v, i) => `${i}:${(v ?? 0).toFixed(2)}`)
      .join(' '),
  )
  ...
  console.log(
    `[hint] hinted ${n} pts:`,
    hinted.points
      .map((p, i) => `[${i}](${p.x.toFixed(2)},${p.y.toFixed(2)})`)
      .join(' '),
  )
} catch (e) {
